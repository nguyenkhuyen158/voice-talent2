...
export default function PartnersManagement() {
  ...
  const filterSuggestions = (input: string) => {
    if (!input) return [];
    const existingDescriptions = partners.map(p => p.description);
    return existingDescriptions.filter(desc => 
      desc.toLowerCase().includes(input.toLowerCase()) && 
      desc.toLowerCase() !== input.toLowerCase()
    );
  };
  ...
  const handleSelectSuggestion = (suggestion: string) => {
    ...
    if (form) {
      const input = form.querySelector('input[name="description"]') as HTMLInputElement;
      if (input) {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {showSuggestions && suggestions.length > 0 && (
                  <div className="mt-2 rounded-md bg-gray-800 p-2 shadow-lg">
                    {suggestions.map((suggestion, index) => (
                      <div
                        key={index}
                        onClick={() => handleSelectSuggestion(suggestion)}
                        className="cursor-pointer rounded-md p-2 hover:bg-gray-700 transition-colors"
                      >
                        {suggestion}
                      </div>
                    ))}
