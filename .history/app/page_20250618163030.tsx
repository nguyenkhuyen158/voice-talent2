...
import { Project, Partner } from '@/data/projects';
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 items-center justify-items-center">
              {partners.map((partner, idx) => (
                <div
                  key={idx}
                  className="w-full max-w-[200px] aspect-[3/2] glass-card p-4 flex flex-col items-center justify-center hover:scale-105 transition-transform group"
                >
                  {partner.logo ? (
                    <img 
                      src={partner.logo} 
                      alt={partner.name}
                      className="w-16 h-16 object-contain mb-2 opacity-70 group-hover:opacity-100 transition-opacity"
                    />
                  ) : (
                    <span className="text-xl font-bold text-white/70 text-center mb-2">
                      {partner.name}
                    </span>
                  )}
                  <p className="text-sm text-white/50 text-center group-hover:text-white/90 transition-colors">
                    {partner.description}
                  </p>
                </div>
              ))}
