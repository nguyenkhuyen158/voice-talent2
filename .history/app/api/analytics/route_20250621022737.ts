...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch analytics', message: error?.message, stack: error?.stack }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error('Analytics API error:', error);
    return NextResponse.json({ error: 'Failed to add visit', message: error?.message, stack: error?.stack }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    console.error('Analytics API error:', error);
    return NextResponse.json({ error: 'Failed to update dailyStats', message: error?.message, stack: error?.stack }, { status: 500 });
  }
}
